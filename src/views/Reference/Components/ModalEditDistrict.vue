<template>
  ...
  >
    <template #header>
      <p class="text-xl font-bold text-neutral-900">Ubah Kecamatan</p>
    </template>
    <template #body>
...
const handleSave = async () => {
  ...
  try {
    ...
    let response = await axios.patch("/api/regions/districts/" + props.data.id, payload);
...
watch(
  ...
  (val) => {
    if (val && props.data) {
      ...
      if (props.data.regencyId) {
        form.value.regency = refRegency.value.find(p => p.id === props.data.regencyId);
      }
    }
  }
);
