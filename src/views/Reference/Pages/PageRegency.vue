    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      <div>
        ...
        <nav class="flex text-xs font-semibold text-neutral-400 mb-2 gap-2 items-center">
          ...
          <span class="text-neutral-600 font-bold text-primary">Kabupaten & Kota</span>
        </nav>
        <h1 class="text-2xl font-bold text-neutral-900 tracking-tight">Data Kabupaten & Kota</h1>
      </div>
    </div>
    ...
    <div class="flex flex-col gap-4">
      ...
      <Table :rows="tableRows" :columns="tableColumns" :loading="loading" @handleSort="handleSort">
        <template #table-content="{ column, row, index }">
          ...
          <span v-else-if="row.field === 'type'">
            <Badge
              :variant="column.type === 'KOTA' ? 'warning' : 'secondary'"
              :text="column.type"
              pill
            />
          </span>
          <span v-else-if="row.field === 'province'">
...
import ModalAddRegency from "../Components/ModalAddRegency.vue";
import ModalEditRegency from "../Components/ModalEditRegency.vue";
...
const handleDelete = (column) => {
  openModalConfirm({
    message: "Apakah yakin ingin menghapus Kabupaten/Kota ini?",
    type: "delete",
    confirmButtonText: "Hapus",
  });
  handleConfirm(async () => {
    try {
      let response = await axios.delete("/api/regions/regencies/" + column.id);
      if (response.status === 200) {
        openModalInfo({
          message: "Berhasil hapus Kabupaten/Kota",
          type: "success",
        });
      }
      getList();
...
const getList = async () => {
  ...
  try {
    ...
    let response = await axios.get(`/api/regions/regencies${params}`);
