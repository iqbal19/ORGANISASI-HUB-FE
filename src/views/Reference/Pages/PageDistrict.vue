    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      <div>
        ...
        <nav class="flex text-xs font-semibold text-neutral-400 mb-2 gap-2 items-center">
          ...
          <span class="text-neutral-600 font-bold text-primary">Kecamatan</span>
        </nav>
        <h1 class="text-2xl font-bold text-neutral-900 tracking-tight">Data Kecamatan</h1>
      </div>
    </div>
    ...
    <div class="flex flex-col gap-4">
      ...
      <Table :rows="tableRows" :columns="tableColumns" :loading="loading" @handleSort="handleSort">
        <template #table-content="{ column, row, index }">
          ...
          <span v-else-if="row.field === 'regency'">
            {{ column.regency?.name || '-' }}
          </span>
          <span v-else-if="row.field === 'province'">
            {{ column.regency?.province?.name || '-' }}
          </span>
...
import ModalAddDistrict from "../Components/ModalAddDistrict.vue";
import ModalEditDistrict from "../Components/ModalEditDistrict.vue";
...
const handleDelete = (column) => {
  openModalConfirm({
    message: "Apakah yakin ingin menghapus Kecamatan ini?",
    type: "delete",
    confirmButtonText: "Hapus",
  });
  handleConfirm(async () => {
    try {
      let response = await axios.delete("/api/regions/districts/" + column.id);
      if (response.status === 200) {
        openModalInfo({
          message: "Berhasil hapus Kecamatan",
          type: "success",
        });
      }
      getList();
...
const getList = async () => {
  ...
  try {
    ...
    let response = await axios.get(`/api/regions/districts${params}`);
