    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      <div>
        ...
        <nav class="flex text-xs font-semibold text-neutral-400 mb-2 gap-2 items-center">
          ...
          <span class="text-neutral-600 font-bold text-primary">Jabatan</span>
        </nav>
        <h1 class="text-2xl font-bold text-neutral-900 tracking-tight">Data Jabatan</h1>
      </div>
    </div>
...
import ModalEditPosition from "../Components/ModalEditPosition.vue";
...
const handleDelete = (column) => {
  openModalConfirm({
    message: "Apakah yakin ingin menghapus Jabatan ini?",
    type: "delete",
    confirmButtonText: "Hapus",
  });
  handleConfirm(async () => {
    try {
      let response = await axios.delete("/api/positions/" + column.id);
      if (response.status === 200) {
        openModalInfo({
          message: "Berhasil hapus Jabatan",
          type: "success",
        });
      }
      getList();
...
const getList = async () => {
  ...
  try {
    ...
    let response = await axios.get(`/api/positions${params}`);
