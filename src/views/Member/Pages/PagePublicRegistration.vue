<template>
  <div class="flex flex-col gap-6 max-w-4xl mx-auto w-full pt-8 pb-24 px-4 sm:px-6">
    <!-- Page Header -->
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div class="flex items-center gap-4">
        <div v-if="orgDetails?.logo" class="w-16 h-16 rounded-xl border border-neutral-200 overflow-hidden bg-white shadow-sm flex-shrink-0">
          <img :src="orgDetails.logo" :alt="orgDetails.name" class="w-full h-full object-cover" />
        </div>
        <div v-else class="w-16 h-16 rounded-xl border border-neutral-200 bg-primary/10 flex items-center justify-center text-primary font-bold text-xl flex-shrink-0">
        ...
        <div>
          ...
          <p class="text-base text-primary font-semibold mt-1">
            {{ orgDetails?.name || 'Memuat...' }}
          </p>
        </div>
      </div>
    </div>
    ...
    <form @submit.prevent="submitForm" class="flex flex-col gap-6">
        ...
        <div class="bg-white border border-neutral-200 rounded-xl shadow-sm">
          ...
          <div class="p-6 flex flex-col gap-6">
            ...
            <div v-if="form.level && form.level !== 'PUSAT'" class="grid grid-cols-1 sm:grid-cols-3 gap-4 animate-fade-in">
              ...
              <div v-if="form.level === 'RANTING'" class="relative">
      ...
      <div class="fixed bottom-0 left-0 right-0 bg-white border-t border-neutral-200 p-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] z-40 flex justify-between gap-3 px-4 sm:px-8">
        <div>
        ...
        <div class="flex gap-2">
          ...
          <BaseButton v-if="currentStep === 3" type="submit" :loading="loading">
            <IconDeviceFloppy class="w-4 h-4 mr-2" /> Simpan Registrasi
          </BaseButton>
        </div>
      </div>
...
const submitForm = async () => {
  ...
  try {
    ...
    if (isEdit.value) {
      ...
      payload.organizationId = route.query.orgId;
    }
    ...
    if (isEdit.value) {
      ...
      const res = await axios.post("/api/members/public/register", payload);
      const data = res.data?.data || res.data;
      openModalInfo({ message: "Berhasil mendaftar. Melanjutkan ke pembayaran...", type: "success" });
      router.push(`/checkout?memberId=${data.id}&orgId=${route.query.orgId}`);
    }
  } catch (error) {
    console.error(error);
    openModalInfo({
...
const fetchFieldConfigs = async () => {
  try {
    ...
    const res = await axios.get(`/api/organizations/${orgId}/field-configs/public`);
...
onMounted(async () => {
  const orgId = route.query.orgId;
  if (!orgId) {
    openModalInfo({ message: "Link pendaftaran tidak valid (orgId hilang)", type: "error" });
    return;
  }

  try {
    const res = await axios.get(`/api/organizations/public/${orgId}`);
    orgDetails.value = res.data?.data || res.data;
  } catch (error) {
    console.error("Gagal mengambil detail organisasi", error);
  }

  // Set form defaults for public registration
  form.value.organizationId = orgId;
  form.value.level = 'RANTING'; // Set default level for public members but they can change it
